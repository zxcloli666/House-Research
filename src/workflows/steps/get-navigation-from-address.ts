...
async function runStep(
  address: string,
): Promise<{ stops: string; routes: string[] }> {
  ...
  for (const toRoute of toRoutes) {
    promises.push((async () => {
      ...
      const route = await safeFreeAi(
        parseMapRoutes,
        `[TONAME]${toRoute.name}[/TONAME]\n[JSON]${jsonRoutes}[/JSON]`,
      );
      log.debug("route...");
      return route;
    })());
...
async function getStopsInRadius(
  lat: string | number,
  lon: string | number,
): Promise<string> {
...
async function getRouteToLocation(
