...
| `GEMINI_MODEL` / `GEMINI_MODEL_LITE` | ❌ | По умолчанию `gemini-flash-lite-latest` (у Pro-моделей на free tier обычно 0 бесплатных запросов). |
...
Если Gemini начинает отваливаться рейт-лимитом сразу после запуска —
почти наверняка используется модель, для которой на вашем аккаунте нет
бесплатной квоты (например Pro), либо `GEMINI_RPM` выставлен выше
реального лимита. Зайдите в Google AI Studio → Rate Limits, посмотрите
актуальные RPM/RPD по моделям и подставьте их в `.env`.
