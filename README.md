...
зашит список Pro → Flash → Flash-Lite (алиасы `-latest`, сами всегда
указывают на актуальную версию). Каждый запрос сначала пробует лучшую
модель на всех ваших ключах по очереди, и только если ни один ключ не
справился — переходит к модели похуже. Если Gemini отваливается рейт-
лимитом сразу после запуска — скорее всего `GEMINI_RPM` выставлен выше
реального лимита; посмотрите свои настоящие RPM в Google AI Studio →
Rate Limits и поправьте `.env`.
